...
import learning.part3testing.InterceptingLogsSpec.{Checkout, CheckoutActor}
import org.scalatest.BeforeAndAfterAll
import org.scalatest.wordspec.AnyWordSpecLike

class InterceptingLogsSpec extends TestKit(ActorSystem("InterceptingLogs", ConfigFactory.load().getConfig("interceptingLogMessages"))) // creating a logger that EventFilter can intercept
  with ImplicitSender
  ...
  "A checkout flow" should {
    "correctly log the dispatch of an order" in {
      // reading the logs for the event
      EventFilter.info(pattern = s"Order [0-9]+ for item $item has been dispatched", occurrences = 1) intercept { // regex for number [0-9]+
        val checkoutRef = system.actorOf(Props[CheckoutActor])
        checkoutRef ! Checkout(item, creditCard)
      }
    }
  }
  "freak out if payment is denied" should {
...
object InterceptingLogsSpec {
  /** Intercepting Logs is useful in integration tests when its very hard to inject probes */
  ...
  class CheckoutActor extends Actor {
    ...
    def pendingPayment(item: String): Receive = {
      ...
      case PaymentDenied => throw new RuntimeException("I can't handle this")// TODO
    }
